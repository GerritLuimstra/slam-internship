...
class WiFiExtendedKalmanFilter:

    def __init__(self, L, R, Q, dt=0.1):
        """
        A simple EKF implementation using a Range sensor with known data association
    ...
    def step(self, u, z):
        ...
        for (z, l) in z:
            ...
            # Correction step
            inv = np.linalg.inv(np.array([[H.dot(sigma_prime.dot(H.T)) + self.Q]]))[0][0]
            K = sigma_prime.dot(H.T).dot(inv)

            # Update the mean and covariance
            mu_prime    += K.dot(z - z_hat)
